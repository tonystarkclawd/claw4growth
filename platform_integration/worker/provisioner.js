  ...
  try {
    ...
    if (onboarding.brand) {
      ...
      await writeFile(`${wsPath}/COMPOSIO.md`, [
        ...
        '- `METAADS_GET_INSIGHTS` — args: `{"ad_account_id":"act_123456","date_preset":"last_30d"}` — campaign performance data',
        ...
        '- `METAADS_UPLOAD_AD_IMAGE` — upload an image for use in ads',
        '- `METAADS_PAUSE_CAMPAIGN` — pause a running campaign',
        '- `METAADS_RESUME_CAMPAIGN` — resume a paused campaign',
        '- `METAADS_UPDATE_CAMPAIGN` — update campaign settings',
        '- `METAADS_DELETE_CAMPAIGN` — delete a campaign',
        '',
        '**Meta Ads workflow:** Always start with `METAADS_GET_AD_ACCOUNTS` to get the user\'s ad account ID, then use that ID for all other operations.',
        '',
        '### LinkedIn',
        '- `LINKEDIN_CREATE_POST` — args: `{"text":"..."}`',
        '',
        '### Reddit',
        '- `REDDIT_SUBMIT_POST` — args: `{"subreddit":"...","title":"...","text":"..."}`',
        '- `REDDIT_SEARCH` — args: `{"query":"...","subreddit":"..."}`',
        '- `REDDIT_GET_COMMENTS` — args: `{"post_id":"..."}`',
        '',
        '### Stripe',
        ...
        '- `SHOPIFY_GET_ORDERS` — get recent orders',
