...
function generateComposioMd() {
  return [
    ...
    '',
    '- **ALL Google apps** (Ads, Gmail, Calendar, Drive, Docs, Sheets, Analytics) → use `google-bridge.js`',
    '- **Non-Google apps** (Meta, LinkedIn, Reddit, Stripe, Shopify, etc.) → use `composio-bridge.js`',
    '',
    '---',
    '',
    ...
    '- `GOOGLEADS_LIST_CAMPAIGNS` — optional: `{"limit":10}`. All campaigns with metrics',
    '- `GOOGLEADS_GET_CAMPAIGN` — args: `{"campaign_id":"123"}`',
    '- `GOOGLEADS_LIST_AD_GROUPS` — args: `{"campaign_id":"123"}`',
    '- `GOOGLEADS_LIST_ADS` — args: `{"ad_group_id":"456"}`',
    '- `GOOGLEADS_GET_METRICS` — args: `{"campaign_id":"123","start_date":"2025-01-01","end_date":"2025-01-31"}`',
    '- `GOOGLEADS_GET_BUDGETS` — no args',
    '- `GOOGLEADS_LIST_KEYWORDS` — args: `{"ad_group_id":"456"}`',
    '',
    '**Workflow:** `GOOGLEADS_GET_ACCOUNT_INFO` → `GOOGLEADS_LIST_CAMPAIGNS` → drill down',
    '',
    '### Gmail',
    '- `GMAIL_GET_PROFILE` — no args. Email address and stats',
    '- `GMAIL_FETCH_EMAILS` — args: `{"max_results":5,"q":"from:someone subject:test"}`',
    '- `GMAIL_SEND_EMAIL` — args: `{"to":"a@b.com","subject":"Hi","body":"<p>Hello</p>","cc":"","bcc":""}`',
    '- `GMAIL_CREATE_EMAIL_DRAFT` — args: `{"to":"a@b.com","subject":"Hi","body":"Hello"}`',
    '- `GMAIL_SEND_DRAFT` — args: `{"draft_id":"r123"}` — send an existing draft',
    '- `GMAIL_LIST_THREADS` — args: `{"max_results":10,"q":""}`',
    '- `GMAIL_FETCH_MESSAGE_BY_THREAD_ID` — args: `{"thread_id":"abc"}`',
    '- `GMAIL_REPLY_TO_THREAD` — args: `{"thread_id":"abc","to":"a@b.com","body":"Thanks"}`',
    '- `GMAIL_LIST_LABELS` — no args',
    '- `GMAIL_CREATE_LABEL` — args: `{"name":"Campaign Replies"}`',
    '- `GMAIL_MODIFY_LABELS` — args: `{"message_ids":["id1"],"add_labels":["Label_1"],"remove_labels":["INBOX"]}`',
    '- `GMAIL_TRASH_MESSAGE` — args: `{"message_id":"abc"}`',
    '',
    '### Google Calendar',
    '- `GOOGLECALENDAR_LIST_CALENDARS` — no args. All calendars',
    '- `GOOGLECALENDAR_GET_CALENDAR` — args: `{"calendarId":"primary"}`',
    '- `GOOGLECALENDAR_EVENTS_LIST` — args: `{"calendarId":"primary","timeMin":"...","maxResults":20}`',
    '- `GOOGLECALENDAR_CREATE_EVENT` — args: `{"summary":"Meeting","start":{"dateTime":"...","timeZone":"Europe/Rome"},"end":{...}}`',
    '- `GOOGLECALENDAR_UPDATE_EVENT` — args: `{"eventId":"abc","summary":"New Title"}`',
    '- `GOOGLECALENDAR_DELETE_EVENT` — args: `{"eventId":"abc"}`',
    '- `GOOGLECALENDAR_QUICK_ADD` — args: `{"text":"Meeting with John tomorrow at 3pm"}`',
    ...
    '- `GOOGLEDRIVE_COPY_FILE` — args: `{"file_id":"abc","name":"Copy of report"}`',
    '- `GOOGLEDRIVE_DELETE_FILE` — args: `{"file_id":"abc"}`',
    '- `GOOGLEDRIVE_EXPORT` — args: `{"file_id":"abc","mime_type":"application/pdf"}` — export as PDF/CSV',
    '- `GOOGLEDRIVE_SHARE` — args: `{"file_id":"abc","email":"a@b.com","role":"reader"}` — share with someone',
    '',
    '### Google Docs',
    '- `GOOGLEDOCS_CREATE_DOCUMENT` — args: `{"title":"Campaign Brief"}`',
    '- `GOOGLEDOCS_GET_DOCUMENT_BY_ID` — args: `{"document_id":"abc"}`',
    '- `GOOGLEDOCS_BATCH_UPDATE` — args: `{"document_id":"abc","requests":[{"insertText":{"location":{"index":1},"text":"Hello"}}]}`',
    '- `GOOGLEDOCS_REPLACE_TEXT` — args: `{"document_id":"abc","replacements":{"{{NAME}}":"Acme Corp","{{DATE}}":"Jan 2025"}}` — template fill',
    '',
    '**Docs workflow:** Create doc → get its ID → use BATCH_UPDATE to write content, or copy a template and use REPLACE_TEXT to fill variables.',
    '',
    '### Google Sheets',
    '- `GOOGLESHEETS_CREATE_SPREADSHEET` — args: `{"title":"Tracker"}`',
    '- `GOOGLESHEETS_GET_SPREADSHEET_INFO` — args: `{"spreadsheet_id":"abc"}`',
    '- `GOOGLESHEETS_BATCH_GET` — args: `{"spreadsheet_id":"abc","ranges":["Sheet1!A1:Z100"]}` — read data',
    '- `GOOGLESHEETS_BATCH_UPDATE` — args: `{"spreadsheet_id":"abc","data":[{"range":"Sheet1!A1","values":[["Name","Score"]]}]}`',
    '- `GOOGLESHEETS_APPEND` — args: `{"spreadsheet_id":"abc","range":"Sheet1!A1","values":[["New row data"]]}` — add rows without overwriting',
    '- `GOOGLESHEETS_CLEAR_VALUES` — args: `{"spreadsheet_id":"abc","range":"Sheet1!A1:Z100"}`',
    '- `GOOGLESHEETS_CREATE_FROM_JSON` — args: `{"title":"Report","headers":["Name","Email"],"rows":[["Alice","a@b.com"]]}` — create sheet from data',
    '- `GOOGLESHEETS_LOOKUP_ROW` — args: `{"spreadsheet_id":"abc","range":"Sheet1!A:Z","search_column":0,"search_value":"john@example.com"}`',
    '',
    '### Google Analytics (GA4)',
    '- `GOOGLE_ANALYTICS_LIST_ACCOUNTS` — no args. List all accounts',
    '- `GOOGLE_ANALYTICS_LIST_PROPERTIES` — args: `{"account_id":"123456"}`. List properties',
    '- `GOOGLE_ANALYTICS_RUN_REPORT` — args: `{"property_id":"123456","date_ranges":[{"startDate":"30daysAgo","endDate":"today"}],"dimensions":[{"name":"date"}],"metrics":[{"name":"sessions"},{"name":"totalUsers"}]}`',
    '- `GOOGLE_ANALYTICS_REALTIME_REPORT` — args: `{"property_id":"123456","dimensions":[{"name":"country"}],"metrics":[{"name":"activeUsers"}]}`',
    '',
    '**Analytics workflow:** `LIST_ACCOUNTS` → `LIST_PROPERTIES` (get property_id) → `RUN_REPORT` for traffic/conversions data.',
    '',
    '---',
    '',
    ...
    '## Important',
    '',
    '- **ALL Google services** → `google-bridge.js` (Ads, Gmail, Calendar, Drive, Docs, Sheets, Analytics)',
    '- **Everything else** → `composio-bridge.js` (Meta, LinkedIn, Reddit, Stripe, Shopify, etc.)',
