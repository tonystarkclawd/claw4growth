...
- Smart inactivity stop — Vercel cron per spegnere macchine idle (Fly autostop incompatibile con OpenClaw cron/heartbeat)
...
- Google Ads: Basic Access in review (submitted 2026-02-19, 1-3 business days)
- Agent: leak "THOUGHT..." occasionale — investigare se bug OpenClaw o config
- Review + integrate agent skills (social trends, content strategy, reporting) nel workspace iniziale
- Review security audit e applicare le fix
...
1. Smart machine inactivity stop — Vercel cron checks last activity per user, stops machine after X hours idle (Fly autostop incompatible with OpenClaw cron/heartbeat)
...
7. Investigate THOUGHT leaking in agent responses
8. Review + integrate agent skills (social trends, content strategy, reporting) into initial workspace stack — align with actual exec/mcporter commands for each integration
9. Review security audit report (generated by separate agent) and apply findings
...
1. **Skills integration** — review agent skills (social trends, content strategy, reporting) e allinearle ai comandi exec/mcporter reali
2. **Security audit review** — applicare findings dal report generato da agent separato
3. **Google Ads Basic Access** — in review, abilitare dopo approvazione (1-3 giorni)
4. **Smart inactivity stop** — Vercel cron per spegnere macchine idle dopo X ore (no Fly autostop, incompatibile con OpenClaw cron/heartbeat)
5. **Dashboard simplification** — remove "Your Operator" section, add simple status badge
6. **Custom domain** — `*.claw4growth.com` instead of `*.fly.dev`
7. **THOUGHT leaking** — investigate and fix agent printing internal thoughts
